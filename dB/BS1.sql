...
SELECT * FROM EMPLOYEE WHERE EMP_NAME IN('송종기', '박나라');
...
-- 2000년 1월 1일 이전에 입사자중 2000년 1월1일 이후 입사한 사원 중 급여를 가장 높게 받는 사원보다 급여가 많은 사원조회하기
SELECT * 
FROM EMPLOYEE
WHERE HIRE_DATE<'00/01/01'
...
-- RANK() OVER() : 중복값 동일한 번호 정하고 다음 번호 건너뛰고 만듬
-- DENSE_RANK() OVER() : 중복값 동일한 번호 정하고 다음번호 순으로 만듬
SELECT *
FROM (
    SELECT 
...
-- DDL
-- 데이터를 정의하는 언어 오랔르에서 사용하는 객체를 생성 , 수정 삭제할때 사용하는 명령어
...
-- 오라클 자료형
-- 숫자형 : NUMBER
-- 문자형 : CHAR, VARCHAR2, NCHAR, NAVARCHAR2
-- 날짜형 : DATE, TIMESTAMP
...
-- 한글자 2BYTE차지, EXPRESS는 영문 1, 한글은 3

-- 숫자자료형 : 정수, 실수, 음수, 양수 모두 저장
-- 사용
-- NUMBER
-- NUMBER(PRECISTION, SCALE)
-- PRECISION : 표현할 수 있는 전체 자리수(1~38)
-- SCALE : 소수점 이하 자리수(-84~127)
CREATE TABLE TBL_NUMBER(
    A NUMBER,
    B NUMBER(5), -- 정수 5자리
    C NUMBER(5,1), -- 소숫점 1자리 포함 5자리
    D NUMBER(5,-1) -- 소숫점 기준 앞(-1) 0으로 만들고 출력
);
...
INSERT INTO TBL_NUMBER VALUES('1234.123','1234.123','1234.123','1234.123'); -- 자동형변환 
...
-- 생성된 테이블 컬럼에 설명작성하기
COMMENT ON COLUMN TBL_MEMBER.MEMBER_ID IS '회원아이디 최소 8글자 이상 작성'; -- 해당컬럼에 코멘트 달기
SELECT *
FROM USER_COL_COMMENTS
WHERE TABLE_NAME='TBL_MEMBER'; -- 코멘트 확인하는법
-- 테이블에 대한 설명작성하기
...
-- CHECK(C) : 컬럼에 저장된 데이터를 조건에 따라 지정
...
FROM USER_CONSTRAINTS JOIN USER_CONS_COLUMNS USING(CONSTRAINT_NAME); -- 보통 이걸로 제약조건 확인함
...
-- 두개이상의 컬럼에 대해 UNIQUE 제약조건을 설정할 수 있다.
CREATE TABLE UQ_MEMBER3(
    MEMBER_ID VARCHAR2(30),
    MEMBER_FWD VARCHAR2(30),
    MEMBER_NAME VARCHAR2(20),
    UNIQUE(MEMBER_ID, MEMBER_NAME) --두개 묶어서 하나로 중복값 확인
);
...
INSERT INTO PK_MEMBER1 VALUES(1, 'ADMIN', '1234', '관리자');
INSERT INTO PK_MEMBER1 VALUES(2, 'USER', '1234', '유저');
INSERT INTO PK_MEMBER1 VALUES(1, 'USER', '1111', '유저1');
INSERT INTO PK_MEMBER1 VALUES(NULL, 'USER', '1234', '유저1');
...
CREATE TABLE BOARD_COMMENT( -- 위에 BOARD(게시판)에 1번글에 COMMENT(댓글)
    BOARD_COMMENT_NO NUMBER PRIMARY KEY,
    BOARD_COMMENT_CONTENT VARCHAR2(800) NOT NULL,
    BOARD_COMMENT_DATE DATE,
    BOARD_REF NUMBER REFERENCES BOARD(BOARD_NO) -- 몇번째 게시판글에 댓글인제 매칭 (참조컬럼은 한개만가능)
);

INSERT INTO BOARD VALUES(1, '첫번째 게시글', '나의 첫 게시글', 'ADMIN', SYSDATE);
INSERT INTO BOARD VALUES(2, '두번째 게시글', '나의 두 게시글', 'ADMIN', SYSDATE);
SELECT * FROM BOARD;
...
-- UNIQUE 제약조건 설정된 컬럼에 NULL값 넣기 -> NULL 값은 중복체크 못한다
-- UNIQUE 제약조건이 설정된 컬럼에 NOT NULL제약조건까지 설정을 해야한다
SELECT * FROM UQ_MEMBER;
...
-- FK제약 조건을 설정했을때 NULL값 - NOT NULL 제약조건을 추가로 설정
-- NULLABLE이 되어 있으면 선택
-- NOT NULL이 되어 있으면 필수
...
-- 부모테이블에 있는 ROW는 함부로 삭제할 수 없다 * 참조하는 자식테이블의 ROW가 있는경우 !
...
-- FK로 다른 테이블의 컬럼값을 설정할때는 반드 설정 대상컬럼은 중복값을 가지면 안됨 -> UNIQUE, PRIMARY KEY 제약조건 설정해야함 
CREATE TABLE FK_BUY(
...
SELECT * FROM FK_BUY;
DELETE FROM FK_PRODUCT WHERE PRODUCT_NO=1; -- 노트북을 지우려고 하는데 FK_BUY에서 노트북 사용중
DELETE FROM FK_PRODUCT WHERE PRODUCT_NO=3;

-- 참조관계에 있는 부모테이블 삭제시 옵션을 사용할 수 있다
-- ON DELETE CASCADE||SET NULL
-- CASCADE : 부모데이터가 지워지면 자식데이터도 같이삭제
-- SET NULL : 참조러럼에 NULL값 저장 * FK설정시 NOT NULL제약조건이 설정되면 안됨
DROP TABLE FK_BUY;
CREATE TABLE FK_BUY(
    BUY_NO NUMBER PRIMARY KEY,
    MEMBER_ID VARCHAR2(20) REFERENCES UQ_MEMBER4(MEMBER_ID),
--    PRODUCT_NO NUMBER REFERENCES FK_PRODUCT(PRODUCT_NO) ON DELETE CASCADE
--    PRODUCT_NO NUMBER REFERENCES FK_PRODUCT(PRODUCT_NO) ON DELETE SET NULL --NOT NULL 넣음 안됨
    PRODUCT_NO NUMBER NOT NULL, --REFERENCES FK_PRODUCT(PRODUCT_NO) ON DELETE SET NULL --NOT NULL 넣음 안됨
    FORING KEY(PRODUCT_NO) REFERENCES FK_ROPDUCT(PRODUCT_NO)
);
DROP TABLE FK_BUY;
...
INSERT INTO CK_MEMBER VALUES('ADMIN', '1234', '여', -19);
INSERT INTO CK_MEMBER VALUES('ADMIN', '1234', '여', NULL);

-- 컬럼에 DEFAULT 값을 설정할 수 있음
...
-- 이름설정하지 않으면 SYS_0000 오라클이 자동으로 제약조건명을 설정
CREATE TABLE CONS_NAME(
    TEST_NO NUMBER CONSTRAINT PK_CONSNAME_TESTNO PRIMARY KEY,
    TEST_NAME VARCHAR2(20) CONSTRAINT NN_TESTNEM NOT NULL
);
SELECT * FROM USER_CONSTRAINTS
...
AS SELECT * FROM EMPLOYEE JOIN DEPARTMENT ON DEPT_CODE=DEPT_ID JOIN JOB USING(JOB_CODE);

SELECT * FROM EMP_ALL_COPY;
-- 서브쿼리로 생성한 테이블은 NOT NULL 제약조건 이외의 설정ㅈ된 제약조건은 생략한다
-- 필요한 제약조건은 추가로 설정을 해야한다

-- 컬럼만 복사하여 새로우 테이블을 생성하려면
...
-- DML에 대해 알아보자
-- INSERT 문 활용하기
-- INSERT문 테이블에 한개 데이터(ROW) 추가하는 명령어
-- 한번에 한개의 데이터만 삽입이 가능
-- 방법
-- 1. 전체컬럼에 대해 값을 대입하는 것
-- 2. 대입원하는 컬럼을 지정하여 대입하는 것 * 지정되지 않은 컬럼은 NULL값 또는 DEFAULT 값 대입,
--      **지정되지 않은 값에 NOT NULL제약조건이 설정되어 있으면 대입이 불가능함
...
-- 컬럼을 지정해서 값 넣기
INSERT INTO TEMP_DEPT (DEPT_ID,DEPT_TITLE) VALUES('D2', '메가스터디');
INSERT INTO TEMP_DEPT (DEPT_ID,LOCATION_ID) VALUES('D2', 'L2');
DESC TEMP_DEPT;
SELECT * FROM TEMP_DEPT;

-- 지정하지 않은 컬럼에 NOT NULL 제약조건이 설정되어 있더라도 DEFAULT 값이 설정되어있다면 실행이 가능
